...
int main(int argc, char *argv[])
{
    ...
    ssh_key key;
    if (ssh_get_server_publickey(ssh, &key) != SSH_OK) { //tls?
    ...
    unsigned char *hash;
    size_t hash_len;
    if (ssh_get_publickey_hash(key, SSH_PUBLICKEY_HASH_SHA1, //hash?
    ...
    ssh_print_hexa(SSH_PUBLICKEY_HASH_SHA1, hash, hash_len); 
